...
export class FollowCamera{
 ...
 pulse(amount=.18,duration=.45){
  this.cinematic=Math.max(this.cinematic,duration);
 ...
 follow(x,y,dt,vx=0,vy=0){
  ...
  const cinematicBoost=this.cinematic>0?Math.sin((this.cinematic/.45)*Math.PI)*3.5:0;
  const targetFov=this.baseFov+speed*4+(Math.abs(vy)>6?1.5:0)+cinematicBoost;
