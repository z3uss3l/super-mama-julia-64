...
export class Particles{
 ...
 burst(pos,color=0xffffff,count=10,speed=4){
  for(let i=0;i<count;i++){
   const m=new THREE.Mesh(new THREE.BoxGeometry(.08,.08,.08),new THREE.MeshBasicMaterial({color,transparent:true}));
 ...
 update(dt){
  for(let i=this.pool.length-1;i>=0;i--){
   const p=this.pool[i];p.userData.life-=dt;p.position.addScaledVector(p.userData.v,dt);p.userData.v.y-=12*dt;p.scale.multiplyScalar(.94);
