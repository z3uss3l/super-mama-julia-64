...
export class Game{
 ...
 startLevel(index,newRun=false){
  ...
  this.player={x:this.state.checkpoint?.x??0,y:this.state.checkpoint?.y??1,vx:0,vy:0,facing:1,grounded:false,jumps:0,health:1,inv:0,attack:0,dash:0,shield:this.state.save.unlocks.shield?3:0,star:0,lion:!!this.state.save.unlocks.lion,contactCooldown:0};
 ...
 attack(){
  ...
  this.player.attack=.22;this.audio.hit();
  const facing=this.player.vx!==0?Math.sign(this.player.vx):(this.player.facing||1);
  const reach=1.35,px=this.player.x+facing*1.05;
  for(const e of this.enemyRuntime){
   if(!e.alive)continue;
   const dx=Math.abs(e.mesh.position.x-px),dy=Math.abs(e.mesh.position.y-this.player.y);
   const extra=e.type==='bat'?.25:e.type==='turret'?-.15:0;
   if(dx<reach+extra&&dy<1.25)this.damageEnemy(e,1);
  }
  if(this.boss){
   const dx=Math.abs(this.boss.mesh.position.x-px),dy=Math.abs(this.boss.mesh.position.y-this.player.y);
   if(dx<1.7&&dy<1.55){
 ...
 damageEnemy(e,amount){
  ...
  e.hitFlash=.12;
  if(e.hp<=0){
   e.alive=false;e.mesh.visible=false;this.state.save.stats.kills++;this.state.addScore(ENEMY_STATS[e.type].value);
   if(this.state.quest.kind==='kills')this.state.quest.progress++;
   this.state.combo++;this.state.comboTimer=GAME.comboWindow
  }
 }
 dash(){
  if(!this.state.save.unlocks.dash||this.player.dash>0)return;
  this.player.dash=.22;this.player.inv=.28;this.player.supportPlatform=null;
  this.player.vx=(this.input.right?1:this.input.left?-1:this.player.facing||1)*GAME.dashSpeed;
  this.state.save.stats.dashes++;this.audio.dash();this.particles.burst(this.playerModel.position,0xffd43b,12,6)
 }
 jump(){
  if(this.player.grounded){
   this.player.vy=GAME.jumpSpeed;this.player.grounded=false;this.player.jumps=1;this.player.supportPlatform=null;
   this.state.save.stats.jumps++;this.audio.jump()
  }else if(this.state.save.unlocks.doubleJump&&this.player.jumps<2){
   this.player.vy=GAME.jumpSpeed*.9;this.player.jumps++;this.audio.jump()
  }
 }
 updateQuest(){
  const q=this.state.quest;if(q.kind==='heart')q.progress=this.state.lives;
  q.done=q.kind==='heart'?this.state.lives>=1:q.progress>=q.target;
 ...
 update(dt){
  ...
  if(p.dash>0)p.dash-=dt;else{
   p.vx=(this.input.right?GAME.playerSpeed:this.input.left?-GAME.playerSpeed:0);
   if(p.vx)p.facing=Math.sign(p.vx);p.vx*=p.lion?1.15:1
  }
  p.vy+=GAME.gravity*dt;p.attack=Math.max(0,p.attack-dt);p.inv=Math.max(0,p.inv-dt);
  p.shield=Math.max(0,p.shield-dt);p.star=Math.max(0,p.star-dt);p.contactCooldown=Math.max(0,p.contactCooldown-dt);
  resolvePlayer(p,this.level.platforms,dt);
  if(p.y<-3.0)this.hurt(true);
  this.playerModel.position.set(p.x,p.y,0);this.playerModel.rotation.y=p.vx<0?Math.PI:0;
  this.playerModel.visible=p.star<=0||Math.floor(p.star*14)%2===0;
  this.lionModel.visible=p.lion;this.lionModel.position.set(p.x-0.8*p.facing,p.y-.05,0);
  this.collectItems();this.updateEnemies(dt);this.updateLion(dt);
  if(!this.state.checkpoint&&p.x>this.level.cfg.length*GAME.checkpointRatio){
   this.state.checkpoint={x:p.x,y:p.y};this.state.persist();this.ui.setCheckpoint(true);this.ui.toast('🚩 CHECKPOINT GESICHERT')
  }
  this.updateBoss(dt);this.updateProjectiles(dt);this.updateHazards();this.particles.update(dt);
  this.state.comboTimer=Math.max(0,this.state.comboTimer-dt);if(this.state.comboTimer===0)this.state.combo=0;
  this.updateQuest();if(p.x>this.level.goalX)this.completeLevel();
  this.follow.follow(p.x,p.y,dt);
  this.ui.setStats({score:this.state.score,coins:this.state.coins,lives:this.state.lives,combo:this.state.combo,level:`${this.level.cfg.name} · ${this.state.level+1}/${LEVELS.length}`});
  this.input.consume()
 }
 collectItems(){
  const p=this.player;
  for(const it of this.items){
   if(!it.alive)continue;it.mesh.rotation.y+=.04;
   const dx=it.x-p.x,dy=it.y-(p.y+.7);
 ...
 updateEnemies(dt){
  ...
  for(const e of this.enemyRuntime){
   ...
   const dx=p.x-e.mesh.position.x,dist=Math.abs(dx);
   const stats=ENEMY_STATS[e.type];
   const aggro=stats.aggro??(e.type==='turret'?10:7);
   if(e.type==='turret'){
    if(dist<aggro&&Math.random()<dt*(stats.fireRate??.65)){
     this.projectile(e.mesh.position.x,e.mesh.position.y,Math.sign(dx)||1,0);
    }
   }else if(e.type==='bat'){
    const targetY=p.y+.9;
    if(dist<aggro)e.mesh.position.x+=Math.sign(dx)*stats.speed*dt*.8;
    e.mesh.position.y=e.y+Math.sin(e.phase*3)*.8+(dist<aggro?Math.max(-.7,Math.min(.7,targetY-e.mesh.position.y))*.12:0);
   }else{
    if(dist<aggro)e.mesh.position.x+=Math.sign(dx)*stats.speed*dt;
    else e.mesh.position.x+=Math.sin(e.phase*.9)*stats.speed*.15*dt;
    e.mesh.position.y=e.y;
   }
   const hitDist=stats.contactRange??.68;
   if(p.contactCooldown<=0&&dist<hitDist&&Math.abs(e.mesh.position.y-p.y)<(stats.contactHeight??1.0)){
    this.hurt(false);p.contactCooldown=.85;
   }
  }
 }
 updateLion(dt){
  if(!this.player.lion)return;
  this.player.lionAttack=(this.player.lionAttack||0)-dt;if(this.player.lionAttack>0)return;
  let target=null,best=5;
  for(const e of this.enemyRuntime){
   if(!e.alive)continue;const d=Math.abs(e.mesh.position.x-this.player.x);
 ...
 updateBoss(dt){
  ...
  const chaseRange=b.stats.aggro??12;
  if(dist<chaseRange)b.mesh.position.x+=Math.sign(dx)*b.stats.speed*dt*.55;
  b.mesh.position.y=2.3+Math.sin(b.phase*1.8)*.8;b.attackTimer-=dt;
  if(dist<chaseRange&&b.attackTimer<=0){
   const n=Math.min(5,2+Math.floor((b.maxHp-b.hp)/5));
   for(let i=0;i<n;i++)this.projectile(b.mesh.position.x,b.mesh.position.y,Math.sign(dx)||1,(i-(n-1)/2)*.5);
   b.attackTimer=Math.max(.55,1.5-(b.maxHp-b.hp)*.035)
  }
 }
 projectile(x,y,dir,spread=0){
  const m=new THREE.Mesh(new THREE.SphereGeometry(.12,6,5),new THREE.MeshBasicMaterial({color:0xff4d4d}));
  m.position.set(x,y,0);this.scene.add(m);this.objectsPush(m);this.projectiles.push({mesh:m,vx:dir*4.5,vy:spread*2.5,life:4})
 }
 objectsPush(m){this.world.objects.push(m)}
 updateProjectiles(dt){
  for(let i=this.projectiles.length-1;i>=0;i--){
   const q=this.projectiles[i];q.life-=dt;q.mesh.position.x+=q.vx*dt;q.mesh.position.y+=q.vy*dt;q.vy-=2*dt;
 ...
 completeLevel(){
  ...
  if(!this.state.quest.done){
   if(performance.now()-this.lastQuestToast>1200){this.lastQuestToast=performance.now();this.ui.toast(`❗ Quest offen: ${this.level.cfg.quest}`)}return
  }
  if(this.level.boss&&!this.state.bossDefeated){
   if(performance.now()-this.lastQuestToast>1200){this.lastQuestToast=performance.now();this.ui.toast('👹 Besiege zuerst den Boss!')}return
  }
  const t=(performance.now()-this.state.levelStart)/1000;
  this.state.markLevelComplete(this.state.level,t);this.state.addScore(Math.max(0,5000-Math.floor(t*20)));
  if(this.state.level===2)this.state.unlock('shield');if(this.state.level===5)this.state.unlock('lion');
  this.state.persist();this.ui.setAbilities(this.state.save.unlocks);
  this.ui.showScreen('LEVEL GESCHAFFT',`${this.level.cfg.name}\nZeit: ${t.toFixed(1)} s\nScore: ${this.state.score}\nQuest: erfüllt`);
  this.state.mode='pause';setTimeout(()=>this.startLevel(this.state.level+1,false),1100)
 }
